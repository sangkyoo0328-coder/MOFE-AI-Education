...
# 로컬: 7.MultiService/code/multi-session-ref.py → 상위 2단이 저장소 루트
# Streamlit Cloud에서 앱 파일만 루트에 두면 parents[2]가 /mount 등으로 잘못 잡히므로 분기
if (
    THIS_FILE.parent.name == "code"
    and len(THIS_FILE.parents) > 2
    and THIS_FILE.parents[1].name == "MultiService"
):
...
def _resolve_log_dir() -> Path:
    """로컬은 REPO_ROOT/logs, Streamlit Cloud 등 읽기 전용 마운트에서는 /tmp 하위 사용."""
    preferred = REPO_ROOT / "logs"
    try:
        preferred.mkdir(parents=True, exist_ok=True)
        return preferred
    except OSError:
        fallback = Path(tempfile.gettempdir()) / "multi_session_ref_logs"
